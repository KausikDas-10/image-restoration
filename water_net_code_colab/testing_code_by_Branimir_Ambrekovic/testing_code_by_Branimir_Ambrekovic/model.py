...
class T_CNN(object):
 ...
 #   gc_test = cv.cvtColor(gc_test,cv.COLOR_BGR2RGB) / 255
    ...
    b, h ,w , c = result_h.shape
    print('b, h, w, c', b, h, w, c)

    for id in range(0,1):
        result_h0 = result_h[id,:,:,:].reshape(h , w , 3)
        result_h0 = result_h0.squeeze()
        #print('result_h0 Max: \t', np.max(np.clip(result_h0, a_min=0, a_max=1)))
        #print('result_h0 Min: \t', np.min(result_h0))

        result_h0 = np.clip(result_h0, a_min=0, a_max=1)
        # print('result_h0 Max: \t', np.max(result_h0))

        # image_path0 = os.path.join(os.getcwd(),"sample")
        image_path0 = '/content/drive/MyDrive/Water_Net_Code/output'
